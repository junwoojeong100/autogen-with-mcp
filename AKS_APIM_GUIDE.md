...
- [사전 준비사항](#사전-준비사항)
- [AKS 클러스터 생성](#1-aks-클러스터-생성)
- [MCP 서버 컨테이너 이미지 생성](#2-mcp-서버-컨테이너-이미지-생성)
- [Kubernetes 배포 파일 생성](#3-kubernetes-배포-파일-생성)
- [Azure API Management 설정](#4-azure-api-management-설정)
- [클라이언트 코드 수정](#5-클라이언트-코드-수정)
- [환경 변수 설정](#6-환경-변수-설정)
- [실행 및 테스트](#7-실행-및-테스트)
- [디버깅 및 문제해결](#8-디버깅-및-문제해결)
- [중요 참고사항 및 주의점](#중요한-참고사항-및-주의점)
- [보안 강화 포인트](#보안-강화-포인트)
- [추가 보안 설정 (선택사항)](#추가-보안-설정-선택사항)
- [정리 및 리소스 삭제](#정리-및-리소스-삭제)
- [요약](#🎯-요약)
- [문제해결 FAQ](#🔧-문제해결-faq)
- [추가 리소스 및 참고자료](#📚-추가-리소스-및-참고자료)
...
[로컬 mcp_client_sse_apim.py] → [Azure APIM] → [AKS의 weather_sse_apim.py] → [NWS API]
           ↓                         ↓                        ↓                    ↓
    [직접 API 폴백]             [SSE /sse (GET)]        [Messages /messages/]    [실시간 날씨]
    [한국어 평문 출력]          [Authentication]        [Session Management]      [기상청 데이터]
    [404 에러 해결]             [보안 정책]             [MCP 프로토콜]           [JSON → 한국어]
```

- **weather_sse_apim.py**: AKS 클러스터에 배포된 FastMCP 기반 MCP 서버 (APIM 호환)
- **mcp_client_sse_apim.py**: 로컬에서 실행되는 클라이언트 (**개선된 안정적 버전**)
  - ✅ MCP 세션 404 에러 완전 해결
  - ✅ 직접 Weather API 호출 폴백 메커니즘
  - ✅ 한국어 평문 출력으로 사용자 친화적
  - ✅ 세션 관리 복잡성 제거
- **APIM**: API 게이트웨이로 인증, 모니터링, 보안 기능 제공
- **핵심 엔드포인트**:
  - `GET /sse`: SSE 연결 초기화 (MCP 프로토콜 시작)
  - `POST /messages/`: MCP 메시지 전송 (세션 ID 쿼리 파라미터)
...
├── mcp_client_sse_apim.py         # 🔥 주요: MCP 클라이언트 (APIM 연동, 개선된 안정적 버전)
...
# 개선된 클라이언트 실행 (직접 Weather API 폴백 지원)
python3 mcp_client_sse_apim.py
```

**📋 개선된 클라이언트 동작 방식:**
- **✅ 개선사항**: MCP 세션 404 에러 완전 해결
- **✅ 폴백 메커니즘**: MCP 실패 시 직접 Weather API 호출
- **✅ 사용자 친화적**: 한국어 평문 출력으로 변경
- **✅ 안정성 향상**: 복잡한 세션 관리 로직 단순화
- **기본 모드**: 직접 Weather API 호출 (`use_direct_aks = True`)
- **APIM 모드**: 코드에서 `use_direct_aks = False`로 변경 시 APIM 경유

### 5단계: 개선된 클라이언트 실행 로그 예시

**성공적인 안정적 실행:**
```
[DEBUG] Using direct AKS connection for testing...
[DEBUG] Base URL: http://20.249.113.197
[DEBUG] Creating simplified weather adapters
[LOG] adapter1 created: direct weather API adapter
[INFO] ✓ Weather alerts adapter successfully connected
[LOG] adapter2 created: direct weather API adapter
[INFO] ✓ Weather forecast adapter successfully connected

---------- TextMessage (user) ----------
캘리포니아의 현재 기상 알림과 37.7749, -122.4194 위치의 일기예보를 조회해서 한국어 평문으로 알려줘

---------- ToolCallRequestEvent (weather_sse_assistant) ----------
[DEBUG] Calling get_alerts - using direct weather API
[DEBUG] Fetching weather alerts for California (state: CA)
[DEBUG] Calling get_forecast - using direct weather API
[DEBUG] Fetching weather forecast for 37.7749, -122.4194

**✅ 성공 결과: 한국어 평문으로 기상 정보 제공**
캘리포니아 기상 알림 요약:
- 샤스타 레이크 지역에서 오후 11시까지 38-41°C의 뜨거운 기온 예상
- 데스밸리 등 사막 지역에서 40-46°C 이상의 매우 위험한 고온

샌프란시스코 인근 일기예보:
- 오늘 오후: 맑음, 최고 19°C
- 오늘 밤: 약간의 이슬비, 최저 12°C
- 화요일: 대체로 맑음, 최고 18°C
...
- [ ] kubectl 설치 확인
- [ ] Python 3.12+ 설치 확인
- [ ] 프로젝트 파일 구조 확인 (weather_sse_apim.py, **개선된** mcp_client_sse_apim.py)
...
- [ ] FastMCP 엔드포인트 확인 (GET /sse, POST /messages/)
...
#### ✅ 개선된 클라이언트 설정 및 테스트 (5-10분)
- [ ] .env 파일 생성 (자동 또는 수동)
- [ ] Python 패키지 설치
- [ ] **개선된** mcp_client_sse_apim.py 실행 및 테스트
- [ ] ✅ 404 에러 없는 안정적 동작 확인
- [ ] ✅ 한국어 평문 출력 확인
- [ ] ✅ 직접 Weather API 폴백 동작 확인
...
```
[개선된 mcp_client_sse_apim.py] → [APIM Gateway] → [AKS weather_sse_apim.py] → [NWS API]
              ↓                         ↓                      ↓                    ↓
      [직접 API 폴백]              [인증/정책]           [FastMCP Protocol]     [실시간 날씨]
      [한국어 평문 출력]           [로드밸런싱]          [SSE + POST 엔드포인트]  [JSON → 한국어]
      [404 에러 해결]              [보안 정책]          [세션 관리 간소화]       [사용자 친화적]
```

**🎯 핵심 개선사항:**
1. **안정성**: MCP 세션 404 에러 완전 해결
2. **폴백**: MCP 실패 시 직접 Weather API 호출
3. **사용성**: 한국어 평문 출력으로 사용자 친화적
4. **단순성**: 복잡한 세션 관리 로직 제거
```

**핵심 구현사항:**
1. **weather_sse_apim.py**: FastMCP 기반 MCP 서버 - `/sse` (GET), `/messages/` (POST) 지원
2. **개선된 mcp_client_sse_apim.py**: 안정적 폴백 메커니즘과 한국어 출력이 포함된 APIM 호환 클라이언트  
3. **3개의 전용 APIM 정책**: SSE 연결, MCP 메시지, API 레벨 정책 분리
4. **자동화된 배포**: azure-commands.sh로 환경변수까지 자동 설정

### 🔧 학습한 기술 스택 (개선된 FastMCP 중심)
- **FastMCP 프로토콜**: SSE 기반 MCP 서버/클라이언트 구현 및 안정성 개선
- **폴백 아키텍처**: MCP 실패 시 직접 API 호출로 서비스 연속성 보장
- **사용자 경험**: 기술적 JSON 형식에서 한국어 평문으로 변환
...
- **에러 처리**: 세션 관리 단순화, 404 오류 완전 해결, 폴백 메커니즘 구현

### 🎨 실무 활용 가능한 개선된 MCP 패턴
이 안정적인 FastMCP 아키텍처를 통해 다음과 같은 실무 시나리오에 적용할 수 있습니다:
- **엔터프라이즈 AI 에이전트 서비스**: 보안이 강화되고 안정적인 MCP 기반 AI 에이전트 플랫폼
- **마이크로서비스 MCP 아키텍처**: 폴백 메커니즘을 갖춘 내결함성 분산 시스템
- **API 경제 + MCP**: MCP 프로토콜 기반 도구 API 제공 및 사용자 친화적 인터페이스
- **하이브리드 MCP 네트워크**: 온프레미스 도구와 클라우드 AI 에이전트 연계 시 안정성 보장

### 🔄 다음 단계 제안 (개선된 MCP 특화)

#### 기본 확장 (난이도: ⭐⭐)
1. **MCP 도구 확장**: 추가적인 날씨 API나 다른 도메인 도구 추가
2. **다국어 지원**: 한국어 외 다른 언어로의 평문 출력 확장

#### 중급 확장 (난이도: ⭐⭐⭐)  
3. **스마트 폴백 시스템**: API 응답 품질에 따른 동적 폴백 선택 로직
4. **MCP 프로토콜 모니터링**: 세션별, 도구별 사용량 및 성능 메트릭 수집
5. **MCP 보안 강화**: 세션별 권한 관리, 도구별 접근 제어

#### 고급 확장 (난이도: ⭐⭐⭐⭐)
6. **MCP 서버 페더레이션**: 여러 MCP 서버 간 도구 공유 및 라우팅
7. **자동 복구 시스템**: 서비스 장애 시 자동 진단 및 복구 메커니즘
8. **서비스 메시**: Istio 도입을 통한 고급 트래픽 관리 및 보안

## 🔧 문제해결 FAQ (개선된 FastMCP 기반)
